    ...
    PHP_EACH stat_opcode AS stat => code BEGIN
        ...
        PATCH_IF (found AND (~%SOURCE_RES%~ STRING_MATCHES_REGEXP ~WP_.+~)) BEGIN  //exclude new polymorph weapons
            PATCH_IF (type == 1 AND val >= 16) BEGIN //set
                ...
                PATCH_IF (found_exceptional AND (~%stat%~ STR_EQ ~STR~) AND exceptional_val > 0) BEGIN
                ...
                LPF REPLACE_SUBSTRING INT_VAR strref_offset=0x0054 STR_VAR substring_to_replace new_substring always_replace = 1 END
            END
